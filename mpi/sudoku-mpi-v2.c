...
int solve(int* sudoku){
    ...
    while(1){
        ...
        if(result != 1){
            ...
            MPI_Irecv(&recv, 1, MPI_INT, MPI_ANY_SOURCE, TAG_EXIT, MPI_COMM_WORLD, &request_recv);
            flag = 0;
            ...
            if(!flag_enter){
                MPI_Irecv(&recv_hyp, 2, MPI_INT, MPI_ANY_SOURCE, TAG_HYP, MPI_COMM_WORLD, &request_recv);
                flag = 0;
