						...
						case TAG_ASK_JOB:	
							printf("[%d] - tag ask\n", id);
									...
									if(work->head != NULL){
										...
										int msg[2];
										MPI_Recv(&msg,2,MPI_INT, recv[1], TAG_HYP, MPI_COMM_WORLD, &status_recv);
										printf("[%d] received initiation", id);
										int* send_msg = (int*)malloc((v_size+2)*sizeof(int));
				                     	...
				                     	MPI_Send(send_msg, (v_size+2), MPI_INT, msg[1], TAG_HYP, MPI_COMM_WORLD);	
