...
int solve(int* sudoku){
    ...
    int low_value, high_value, number_amount, flag_enter = 1, insert = 1, no_job, flag, data;
    ...
    while(1){
        ...
        if(flag_enter){
            ...
            solved = solve_from(cp_sudoku, r_mask_array, c_mask_array, b_mask_array, work, last_pos);
            if(solved == 1){
                for(i = 0; i < v_size; i++)
